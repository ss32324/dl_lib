...
class BatchNorm(Normalization):
    ...
    def forward(self, inputs):
        ...
        ### 批量均值
        mu = np.mean(inputs, axis=axis, keepdims=True)
        ### 批量方差
        self.sigma = np.mean((inputs - mu)** 2, axis=axis, keepdims=True) ** 0.5 + Normalization.CONST_E
        ### 數值依均值位移後 依方差為準值縮小
    ...
    def backward(self, deviation):
        ...
        ### 更新
        self.gamma_increments = np.mean(deviation*self.z_hat, axis=axis)
        self.beta_increments = np.mean(deviation, axis=axis)
...
class LayerNorm(Normalization):
    ...
    def backward(self, deviation):
        ...
        ### 更新
        self.gamma_increments = np.mean(deviation * self.z_hat, axis=1, keepdims=True)
        self.beta_increments = np.mean(deviation, axis=1, keepdims=True)
